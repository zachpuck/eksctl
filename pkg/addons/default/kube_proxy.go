...
func UpdateKubeProxyImageTag(clientSet kubernetes.Interface, controlPlaneVersion string, dryRun bool) error {
	...
	daemonSetsClient := clientSet.AppsV1().DaemonSets(metav1.NamespaceSystem)
	d, err := daemonSetsClient.Get(KubeProxy, metav1.GetOptions{})
	if err != nil {
	...
	*image = strings.Join(imageParts, ":")
	...
	if _, err := daemonSetsClient.Update(d); err != nil {
		return err
	}
