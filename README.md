...
3. update default add-ons:
  - `kube-system:deployment/kube-dns` or `kube-system:deployment/coredns`
  - `kube-system:ds/kube-proxy`
  - `kube-system:ds/aws-node`
...
Control version updates must be done for one minor version at a time.
...
- review nodegroups you have and which ones can be delete or must be replaced with new version
...
If you chose to deleted definition of old nodegoups from the config file, then you can run:
```
eksctl create nodegroup --config-file=<path>
```

Alternatively, you can pass `--only` with a list of the new nodegroup names that you want to
add (you can use glob expressions, or list specific names - see above for details):
```
...
There 3 default add-ons that get included in each EKS cluster, the process for updating each of them is different, hence
there are 3 distinct commands that you will need to run.

To update `kube-system:ds/kube-proxy`, run:
```
eksctl utils update-kube-proxy
```


To update `kube-system:ds/aws-node`, run:
```
eksctl utils update-aws-node
```

If you have upgraded from 1.10 to 1.11, you will need to replace `kube-system:deployment/kube-dns` with `kube-system:deployment/coredns`.
To do that, run:
```
eksctl utils replace-kube-dns-with-coredns
```

Once upgraded, be sure to run `kubectl get pods -n kube-system` and check if all addon pods are in ready state.
